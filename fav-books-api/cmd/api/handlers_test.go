...
func TestApplication_AllUsers(t *testing.T) {
	...
	var mockedRows = mockedDB.NewRows([]string{"id", "email", "first_name", "last_name", "password",
		"active", "created_at", "updated_at", "has_token"})
	mockedRows.AddRow("1", "me@here.com", "Jack", "Smith", "abc123", "1", time.Now(), time.Now(), "0")

	// tell mock what queries we expected
	...
	// check for expexted status code
