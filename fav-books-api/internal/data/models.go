...
func (u *User) GetAll() ([]*User, error) {
	...
	case 
		when (select count(id) from tokens t where user_id = users.id and t.expiry > NOW() ) > 0 
		then 1
		else 0
	end as has_token
...
// GetUserByEmail <- GetByEmail  returns one user by email
func (u *User) GetUserByEmail(email string) (*User, error) {
	...
	// todo get a token like in GetAll
...
// GetUserByID <- GetOne   returns one user by id
...
// UpdateUser <- Update   updates one user in the database, using the information
...
// InsertUser  inserts a new user into the datbase, and returns the ID of the newly inserted row
...
func (t *Token) GetByToken(plainText string) (*Token, error) {
	...
	q := `select id, user_id, email, token, token_hash, created_at, updated_at, expiry 
	from tokens where token = $1`

	var token Token
	row := db.QueryRowContext(ctx, q, plainText)
	err := row.Scan(
		&token.ID,
		&token.UserID,
		&token.Email,
...
func (t *Token) DeletTokensForUser(id int) error {
	ctx, cancel := context.WithTimeout(context.Background(), dbTimeout)
	defer cancel()
